      ...
      <div className="mx-auto max-w-8xl space-y-4">
        ...
        {loading ? (
          ...
          <>
            ...
            <TableShell title="Circle Analytics" subtitle="Organization-wide headcount and pending document concentration" searchValue={tableState.circle.search} onSearchChange={(value) => updateTableState("circle", { search: value, page: 1 })} columns={[{ key: "label", label: "Circle", sortable: true }, { key: "total_employees", label: "Total Employees", sortable: true }, { key: "active_employees", label: "Active", sortable: true }, { key: "inactive_employees", label: "Inactive", sortable: true }, { key: "pf_pending", label: "PF Pending", sortable: true }, { key: "aadhaar_pending", label: "Aadhaar Pending", sortable: true }, { key: "bank_pending", label: "Bank Pending", sortable: true }, { key: "uan_pending", label: "UAN Pending", sortable: true }, { key: "esic_pending", label: "ESIC Pending", sortable: true }, { key: "completion_percentage", label: "Completion %", sortable: true, render: (row) => `${formatPercent(row.completion_percentage)}` }]} rows={circleTable.rows} onRowClick={(row) => openDrilldown(`Circle — ${row.label}`, "circle", "circle", row.label)} onSortChange={(key) => updateTableState("circle", { sortBy: key, sortOrder: tableState.circle.sortBy === key && tableState.circle.sortOrder === "asc" ? "desc" : "asc", page: 1 })} page={circleTable.page} totalPages={circleTable.totalPages} onPageChange={(page) => updateTableState("circle", { page })} totalRecords={circleTable.totalRecords} emptyMessage="No circle data available for the current filters." loading={false} exportLabel="Export circles" onExport={() => exportAllTables()} />

            <TableShell title="CMP Analytics" subtitle="CMP-level workforce and document backlog" searchValue={tableState.cmp.search} onSearchChange={(value) => updateTableState("cmp", { search: value, page: 1 })} columns={[{ key: "label", label: "CMP", sortable: true }, { key: "total_employees", label: "Total Employees", sortable: true }, { key: "active_employees", label: "Active", sortable: true }, { key: "inactive_employees", label: "Inactive", sortable: true }, { key: "pending_documents", label: "Pending Documents", sortable: true }, { key: "completion_percentage", label: "Completion %", sortable: true, render: (row) => `${formatPercent(row.completion_percentage)}` }]} rows={cmpTable.rows} onRowClick={(row) => openDrilldown(`CMP — ${row.label}`, "cmp", "cmp", row.label)} onSortChange={(key) => updateTableState("cmp", { sortBy: key, sortOrder: tableState.cmp.sortBy === key && tableState.cmp.sortOrder === "asc" ? "desc" : "asc", page: 1 })} page={cmpTable.page} totalPages={cmpTable.totalPages} onPageChange={(page) => updateTableState("cmp", { page })} totalRecords={cmpTable.totalRecords} emptyMessage="No CMP data available for the current filters." loading={false} exportLabel="Export CMPs" onExport={() => exportAllTables()} />

            <TableShell title="Job Role Analytics" subtitle="Role-wise headcount and missing document concentration" searchValue={tableState.jobRole.search} onSearchChange={(value) => updateTableState("jobRole", { search: value, page: 1 })} columns={[{ key: "label", label: "Job Role", sortable: true }, { key: "total_employees", label: "Employee Count", sortable: true }, { key: "missing_documents", label: "Missing Documents", sortable: true }, { key: "completion_percentage", label: "Completion %", sortable: true, render: (row) => `${formatPercent(row.completion_percentage)}` }]} rows={jobRoleTable.rows} onRowClick={(row) => openDrilldown(`Job Role — ${row.label}`, "job_role", "job_role", row.label)} onSortChange={(key) => updateTableState("jobRole", { sortBy: key, sortOrder: tableState.jobRole.sortBy === key && tableState.jobRole.sortOrder === "asc" ? "desc" : "asc", page: 1 })} page={jobRoleTable.page} totalPages={jobRoleTable.totalPages} onPageChange={(page) => updateTableState("jobRole", { page })} totalRecords={jobRoleTable.totalRecords} emptyMessage="No job role data available for the current filters." loading={false} exportLabel="Export roles" onExport={() => exportAllTables()} />
          </>
        )}
      </div>
