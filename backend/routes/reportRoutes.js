  ...
  const jcId =
    ...
    try {
      ...
      const missingColumns = [
        ...
        "total_cnum_count", "total_outage", "availability", "cells_up",
        "cells_up_mod", "vendor", "r4g_availability", "air_fiber_sites", "updated_r4g"
    ...
    await query("DELETE FROM report_uploads WHERE id = ?", [upload.id]);
            ...
            router.post("/upload-bulk", (req, res) => {
              uploadMany.array("files")(req, res, async (err) => {
                ...
                try {
                  const site_category = (req.body.siteCategory || "tower").toLowerCase();
                  const uploadedBy = Array.isArray(req.body.uploadedBy)
                  ? req.body.uploadedBy[0].trim()
                  : (req.body.uploadedBy || "Admin").trim();
            ...
            router.post("/manual-bulk-upload", (req, res) => {
  ...
  GNB_2026-06-15.xlsx`
    ...
    const totalRecords =
      ...
      console.log("typeof =", typeof uploadedBy);
